...
function balanceCommand(argv){
    Promise.all([bittrexPromise.getBalances(), bittrexPromise.getMarketSummaries()]).then(function(data){
        ...
        var table  = new Table({head: ['Coin'.cyan, 'Amout'.cyan, 'Pending' .cyan, 'Rate'.cyan, '% Change'.cyan, 'BTC Value'.cyan, 'USD Value'.cyan]});
        ...
        for(var key in balancesObj){
            ...
            tableRows.push([market.baseCurrency+'-'+market.tradeCurrency, market.Balance, market.Balance - market.Available, market.price.toFixed(8), pc2, btcValue.toFixed(8), '$'+((usdValue).toFixed(2))]);
        }
